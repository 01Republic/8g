# 자동 레이아웃 - 전략 및 최적화

**상위 문서**: [F-010: 자동 레이아웃](../F-010-자동-레이아웃.md)
**최종 수정일**: 2025-11-11

---

## 개요

이 문서는 워크플로우 레이아웃 방향, 패턴별 전략, 성능 최적화, 커스텀 레이아웃을 다룹니다.

---

## 레이아웃 방향

### 1. Top-to-Bottom (TB)

**기본 방향**: 위에서 아래로

```
    [Start]
       ↓
   [Step 1]
       ↓
   [Step 2]
    ↙   ↘
[Step 3] [Step 4]
    ↘   ↙
   [Step 5]
```

**사용 사례**: 일반적인 워크플로우, 세로 스크롤 선호 시

---

### 2. Left-to-Right (LR)

**방향**: 왼쪽에서 오른쪽으로

```
[Start] → [Step 1] → [Step 2] → [Step 3]
                         ↓
                    [Step 4]
```

**사용 사례**: 넓은 화면, 긴 선형 워크플로우

---

### 3. Right-to-Left (RL)

오른쪽에서 왼쪽으로 (RTL 언어 지원 시)

---

### 4. Bottom-to-Top (BT)

아래에서 위로 (특수 케이스)

---

## 워크플로우 패턴별 전략

### 1. 선형 워크플로우

```
[Start] → [Step 1] → [Step 2] → [Step 3] → [End]
```

**특징**: 단순한 순차 실행

**추천 레이아웃**:
- Top-to-Bottom (기본)
- Left-to-Right (넓은 화면)

**설정**:
```typescript
applyDagreLayout(nodes, edges, {
  direction: "TB",
  rankSep: 100,
  nodeSep: 50
});
```

---

### 2. 분기 워크플로우

```
         [Start]
            ↓
         [Step 1]
          ↙   ↘
    [Step 2A] [Step 2B]
          ↘   ↙
         [Step 3]
```

**특징**: 조건부 분기

**추천 레이아웃**: Top-to-Bottom, 중앙 정렬

**설정**:
```typescript
applyDagreLayout(nodes, edges, {
  direction: "TB",
  rankSep: 120,
  nodeSep: 80  // 분기 시 여유 공간
});
```

---

### 3. 병렬 워크플로우 (계획)

```
      [Start]
     ↙   ↓   ↘
[Step 1][Step 2][Step 3]
     ↘   ↓   ↙
       [End]
```

**특징**: 독립적인 병렬 실행

**추천 레이아웃**: 동일 레벨 배치

---

### 4. 복합 워크플로우

```
         [Start]
            ↓
         [Loop]────┐
         ↙   ↘     │
    [Step A] [Step B]
         ↘   ↙     │
       [Merge]     │
           ↓       │
      [Condition]──┘
           ↓
         [End]
```

**특징**: 루프 + 분기 + 조건

**추천 레이아웃**: Dagre 자동 최적화

**설정**:
```typescript
applyDagreLayout(nodes, edges, {
  direction: "TB",
  rankSep: 150,
  nodeSep: 100
});
```

---

## 충돌 방지

### 엣지 겹침 방지

```typescript
// Dagre 고급 설정
g.setGraph({
  rankdir: "TB",
  ranksep: 100,
  nodesep: 50,
  edgesep: 20,      // 엣지 간격
  marginx: 20,      // X축 여백
  marginy: 20       // Y축 여백
});
```

---

### 노드 겹침 방지

```typescript
function ensureNoOverlap(nodes: Node[]): Node[] {
  const occupied = new Set<string>();

  return nodes.map((node) => {
    let { x, y } = node.position;
    let key = `${x},${y}`;

    // 이미 점유된 위치면 조정
    while (occupied.has(key)) {
      x += 10;
      y += 10;
      key = `${x},${y}`;
    }

    occupied.add(key);

    return {
      ...node,
      position: { x, y }
    };
  });
}
```

---

## 반응형 레이아웃

### 화면 크기에 따른 레이아웃

```typescript
function useResponsiveLayout() {
  const [layoutDirection, setLayoutDirection] = useState<"TB" | "LR">("TB");

  useEffect(() => {
    const handleResize = () => {
      const width = window.innerWidth;

      // 넓은 화면: 좌우 배치
      if (width > 1600) {
        setLayoutDirection("LR");
      } else {
        setLayoutDirection("TB");
      }
    };

    window.addEventListener("resize", handleResize);
    handleResize();

    return () => window.removeEventListener("resize", handleResize);
  }, []);

  return layoutDirection;
}
```

---

## 성능 최적화

### 1. 대규모 그래프 처리

```typescript
// 100개 이상 노드 시 최적화
function applyOptimizedLayout(
  nodes: Node[],
  edges: Edge[]
): Node[] {
  if (nodes.length < 100) {
    return applyDagreLayout(nodes, edges);
  }

  // 간소화된 레이아웃
  return applyDagreLayout(nodes, edges, {
    rankSep: 80,
    nodeSep: 40
  });
}
```

**성능 개선**:
- 노드 100개 미만: 일반 레이아웃
- 노드 100개 이상: 간격 축소로 계산 시간 단축

---

### 2. 레이아웃 캐싱

```typescript
const layoutCache = new Map<string, Node[]>();

function getCachedLayout(
  nodes: Node[],
  edges: Edge[]
): Node[] | null {
  const key = JSON.stringify({ nodes: nodes.map(n => n.id), edges });

  if (layoutCache.has(key)) {
    return layoutCache.get(key)!;
  }

  const layoutedNodes = applyDagreLayout(nodes, edges);
  layoutCache.set(key, layoutedNodes);

  return layoutedNodes;
}
```

**효과**: 동일한 그래프 구조 재사용 시 즉시 반환

---

### 3. Web Worker (계획 - Q2 2025)

```typescript
// worker.ts
self.onmessage = (e) => {
  const { nodes, edges } = e.data;
  const layoutedNodes = applyDagreLayout(nodes, edges);
  self.postMessage(layoutedNodes);
};

// main thread
const worker = new Worker("./layout-worker.ts");
worker.postMessage({ nodes, edges });
worker.onmessage = (e) => {
  setNodes(e.data);
};
```

**효과**: UI 블로킹 없이 백그라운드 레이아웃 계산

---

## 커스텀 레이아웃 (향후)

### 원형 레이아웃

```typescript
function applyCircularLayout(nodes: Node[]): Node[] {
  const centerX = 500;
  const centerY = 300;
  const radius = 200;

  return nodes.map((node, index) => {
    const angle = (2 * Math.PI * index) / nodes.length;
    return {
      ...node,
      position: {
        x: centerX + radius * Math.cos(angle),
        y: centerY + radius * Math.sin(angle)
      }
    };
  });
}
```

**사용 사례**: 순환 워크플로우, 상태 머신

---

### 그리드 레이아웃

```typescript
function applyGridLayout(nodes: Node[], columns: number = 3): Node[] {
  const nodeWidth = 250;
  const nodeHeight = 100;
  const gapX = 50;
  const gapY = 50;

  return nodes.map((node, index) => {
    const col = index % columns;
    const row = Math.floor(index / columns);

    return {
      ...node,
      position: {
        x: col * (nodeWidth + gapX),
        y: row * (nodeHeight + gapY)
      }
    };
  });
}
```

**사용 사례**: 병렬 작업 시각화

---

## 알려진 제약사항

### 제약 1: 복잡한 그래프 레이아웃 지연

**현상**: 100개 이상 노드 시 1-2초 소요

**대안**: Web Worker 사용 계획

**계획**: Q2 2025

---

### 제약 2: 수동 배치 후 자동 레이아웃 적용 시 위치 손실

**현상**: 사용자 수동 배치가 초기화됨

**대안**: "레이아웃 초기화" 경고 표시

**계획**: Q1 2025 - 수동 배치 보존 옵션

---

## 향후 로드맵

### Q1 2025
- ✅ 수동 배치 보존 옵션
- ✅ 레이아웃 미리보기
- ✅ 레이아웃 실행 취소

### Q2 2025
- 🔄 Web Worker 백그라운드 레이아웃
- 🔄 커스텀 레이아웃 엔진 (원형, 그리드)
- 🔄 레이아웃 템플릿 저장

---

## 관련 문서

- [F-010: 자동 레이아웃](../F-010-자동-레이아웃.md) - 메인 문서
- [레이아웃 알고리즘](./레이아웃-알고리즘.md) - Dagre 구현 상세
- [React Flow 문서](https://reactflow.dev/)
- [Dagre 문서](https://github.com/dagrejs/dagre/wiki)

---

**문서화 원칙 준수**:
- ✅ 200줄 이하
- ✅ 한글 작성
- ✅ 전략 및 최적화 중심
