...
async function executeStepWithForEach(
  ...
  // 1. 배열 데이터 조회
  const arrayData = JSONPath.query(
    context,
    step.repeat!.forEach!
  )[0];

  if (!Array.isArray(arrayData)) {
    throw new Error(`forEach path did not return an array: ${step.repeat!.forEach}`);
  }
...
async function executeStepWithCount(
  ...
  // count 값 결정 (정적 또는 동적)
  const countValue = typeof step.repeat!.count === "string"
