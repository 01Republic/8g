...
**반환 데이터 구조 (WorkspaceDetailItemDto)**:

```typescript
{
  slug: string;              // 워크스페이스 고유 식별자 (URL-safe)
  displayName: string;       // 표시 이름
  profileImageUrl: string;   // 프로필 이미지 URL
  description: string;       // 워크스페이스 설명
  publicEmail: string;       // 공개 이메일 주소
  billingEmail: string;      // 청구 이메일 주소
  orgPageUrl: string;        // 조직 페이지 URL
