...
const DEFAULT_PLACEHOLDER_ERROR_MESSAGE = 'FormWorkflow 표현식을 평가하는 중 오류가 발생했습니다.'
...
function formatValue(value: unknown, depth: number): string {
  ...
  if (valueType === 'string') {
    ...
    const needsTemplate = stringValue.includes('`') || stringValue.includes('${') || stringValue.includes('\n')
    if (needsTemplate) {
  ...
  if (value && typeof value === 'object') {
    const entries = Object.entries(value as Record<string, unknown>).filter(([, v]) => typeof v !== 'undefined')
...
function toFormWorkflow(value: unknown, previous?: FormWorkflow): FormWorkflow | null {
  if (!value || typeof value !== 'object') {
    return null
  }
  ...
  const targetUrl = typeof candidate.targetUrl === 'string'
    ? candidate.targetUrl
    : previous?.targetUrl

  const parser = typeof candidate.parser === 'function'
...
export function useWorkflowConfig({ index, withMeta, initialWorkflow }: UseWorkflowConfigParams): UseWorkflowConfigResult {
  const [workflowText, setWorkflowText] = useState<string>(() => serializeWorkflow(initialWorkflow))
  const [workflowError, setWorkflowError] = useState<string | null>(null)
  const [workflow, setWorkflow] = useState<FormWorkflow | undefined>(initialWorkflow)

  const initialSerialized = useMemo(() => serializeWorkflow(initialWorkflow), [initialWorkflow])
  ...
  const handleWorkflowChange = useCallback((text: string) => {
    ...
    try {
      ...
      try {
        ...
        const message = expressionError instanceof Error ? expressionError.message : DEFAULT_PLACEHOLDER_ERROR_MESSAGE
