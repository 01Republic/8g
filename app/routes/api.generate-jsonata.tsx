...
export async function action({ request }: ActionFunctionArgs) {
  try {
    ...
    const agent = createAgent({
      ...
      systemPrompt: "You are a JSONata expert. Generate valid JSONata expressions for data transformation.",
      responseFormat: z.object({
        expression: z.string().describe("The JSONata expression to transform the data")
      })
    });
...
Generate a JSONata expression to transform the source data to match the target schema.`
    ...
    return data({ 
      success: false, 
      error: error instanceof Error ? error.message : "Failed to generate JSONata expression" 
