...
import type { CheckboxSectionSchema, AppFormMetadata } from "./types";
  ...
  sections: [
      ...
      uiSchema: {
        ...
        workflow: {
          ...
          parser: (result: any) => {
            ...
            const mapped = (workspaceData as ElementData[]).map(
              (it: ElementData) => ({
            ...
            const nonEmpty = mapped.filter(
              (w) =>
                w.elementId ||
                w.elementText ||
                (w.elementId !== "app" && w.elementId !== "join"),
            );
            const uniqueMap = new Map<
              string,
              { elementId: string; elementText: string }
            >();
            for (const w of nonEmpty) {
              const key =
                (w.elementId && `id:${w.elementId}`) || `text:${w.elementText}`;
    ...
    },
    {
      ...
      uiSchema: {
        ...
        errorMessage:
          "관리자 권한 없음: 워크스페이스 관리자만 접근할 수 있습니다.",
        ...
        workflow: {
          ...
          parser: (result: any) => {
            // @ts-ignore
            const pageTitle = (
              (result.steps[0].result.data as string) || ""
            ).toLowerCase();
          ...
          steps: [
            {
              ...
              block: {
                name: "get-text",
                selector:
                  '[data-qa-column="workspace-members_table_email"] .c-truncate',
                findBy: "cssSelector",
                option: { multiple: true },
              },
              next: "collectStatuses",
            },
            {
              id: "collectStatuses",
              block: {
                name: "get-text",
                selector:
                  '[data-qa-column="workspace-members_table_account_status"] div div',
                findBy: "cssSelector",
                option: { multiple: true },
              },
              next: "collectJoinDates",
            },
            {
              id: "collectJoinDates",
              block: {
                name: "get-text",
                selector:
                  '[data-qa-column="workspace-members_table_created"] .c-table_cell',
                findBy: "cssSelector",
                option: { multiple: true },
              },
            },
