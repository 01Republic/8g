...
export const RecordFieldBlock = (props: RecordFieldBlockProps) => {
  ...
  const handleValueChange = (key: string, newValue: string, cursorPos: number) => {
    const newRecord = { ...recordValue };
        ...
        {entries.map(([key, value], index) => {
          ...
          return (
            <div key={index} className="flex gap-2 items-start">
              <Select
                value={valueType}
                onValueChange={(newType: ValueType) => handleTypeChange(key, newType)}
              >
              ...
              <Input
                placeholder="키"
                value={key}
                onChange={(e) => handleUpdateEntry(key, e.target.value, valueStr)}
                className="flex-1"
              />
